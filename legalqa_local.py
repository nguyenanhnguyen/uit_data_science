...
===============================================================================
BẢN SỬA #7 (tái lập được kết quả + ablation warmup.json + sổ thí nghiệm — theo phân tích
chênh lệch điểm số .py-vs-Kaggle, xem PHAN_TICH_KY_THUAT.md)
===============================================================================
1. SEED toàn cục (`SEED=42`, hàm `set_all_seeds()`) gọi NGAY ĐẦU `main()` — trước đây
   `random.seed(42)` chỉ đặt ở Bước 6 (trước lúc lấy mẫu dev-eval), còn Bước 4
   (`random.sample` chọn 3000 câu train, `random.choice` chọn hard-negative dự phòng) chạy
   TRƯỚC đó với random state KHÔNG seed — nghĩa là 2 lần chạy cùng 1 file, cùng 1 máy vẫn
   fine-tune trên 2 tập con khác nhau mỗi lần, làm mọi so sánh không đáng tin. Dòng
   `random.seed(SEED)` ở Bước 6 VẪN GIỮ (re-seed có chủ đích, không phải sót lại): đảm bảo
   300 câu dev-eval luôn cố định bất kể USE_WARMUP/kết quả Bước 4 thế nào.
2. `USE_WARMUP` (mặc định True): gộp `warmup.json` (nếu tồn tại cạnh script, đúng schema
   `{qid: {"question": str, "answer": str}}`) vào dữ liệu fine-tune Bước 4 — giống hành vi
   bên bản Kaggle. Đặt `False` để ablation: so sánh có/không warmup ở CÙNG seed.
3. Sổ thí nghiệm `EXPERIMENT_LOG_PATH` (mặc định `experiment_log.jsonl` cạnh script): mỗi
   lần `main()` chạy xong ghi THÊM (append, không ghi đè) 1 dòng JSON gồm seed, cấu hình
   warmup, số bước fine-tune thật đã chạy, cấu hình dev-eval chọn được, METEOR/ROUGE-L,
   Recall@k. Trước đây các con số này chỉ in ra console rồi mất, không so sánh lại được
   giữa nhiều lần chạy.
...
WARMUP_PATH = HERE / "warmup.json"     # SỬA (bản #7): tuỳ chọn — gộp thêm nếu tồn tại VÀ
                                        # USE_WARMUP=True. Không có file này thì bỏ qua êm,
                                        # không crash (đúng hành vi bên bản Kaggle).
PUBLIC_PATH = HERE / "public-official.json"
OUT_DIR = HERE
EXPERIMENT_LOG_PATH = OUT_DIR / "experiment_log.jsonl"  # "sổ thí nghiệm" — 1 dòng JSON/lần chạy

# ---- BẢN SỬA #7: seed toàn cục + cờ bật/tắt warmup để ablation được ----------------
# Phát hiện từ phân tích chênh lệch điểm .py-vs-Kaggle: random.sample/random.choice ở Bước 4
# (chọn tập con để fine-tune, chọn hard-negative dự phòng) chạy TRƯỚC dòng random.seed(42) cũ
# (vốn chỉ đặt ngay trước lúc lấy mẫu dev-eval ở Bước 6) — nghĩa là HAI LẦN CHẠY CÙNG MỘT
# FILE, CÙNG MỘT MÁY vẫn fine-tune trên hai tập con 3000 câu KHÁC NHAU mỗi lần, làm mọi so
# sánh (kể cả so sánh với chính nó) không đáng tin. Từ bản này: seed toàn cục được đặt NGAY
# ĐẦU main() (xem set_all_seeds()), trước Bước 3 — làm cho việc chọn tập train + hard-negative
# tái lập được. Dòng random.seed(SEED) ở Bước 6 (trước khi lấy mẫu dev-eval) VẪN GIỮ NGUYÊN
# thêm một lần nữa — mục đích khác: đảm bảo 300 câu dev-eval LUÔN CỐ ĐỊNH bất kể USE_WARMUP/
# USE_FINETUNE bật hay tắt (số lời gọi random.* trước đó thay đổi tuỳ cấu hình sẽ làm lệch
# state ngẫu nhiên nếu không re-seed ở đây) — cần thiết để ablation (mục 5.2 trong file phân
# tích kỹ thuật) so sánh đúng nghĩa "cùng 300 câu, chỉ khác 1 biến".
SEED = 42
...
def finetune_or_load_dense(train_positive, train_data, chunk_by_id, all_chunks, bm25):
    ...
    # BẢN SỬA #7: trả thêm `finetune_info` (dict) để main() ghi vào sổ thí nghiệm
    # (EXPERIMENT_LOG_PATH) — trước đây các con số này chỉ in ra console rồi mất, không
    # so sánh lại được giữa các lần chạy.
    finetune_info = {"used_finetune": False, "reason": None, "n_pairs_available": len(train_positive),
                      "n_pairs_used": 0, "max_steps": 0, "mini_batch_size_final": None, "calib_time_s": None}
    ...
    if not use_finetune:
        ...
        print(f"  {reason} -> dùng zero-shot '{BASE_DENSE_MODEL}', không fine-tune.")
    ...
    try:
        ...
        finetune_info["reason"] = f"thiếu package: {e}"
        return model, finetune_info
    ...
    if len(train_positive) > MAX_TRAIN_EXAMPLES:
        ...
        print(f"  Có {len(train_positive)} positive pairs, lấy mẫu ngẫu nhiên {MAX_TRAIN_EXAMPLES} "
              ...
              f"Tái lập được nhờ SEED={SEED} (xem set_all_seeds() ở CONFIG).")
    else:
    ...
    for attempt in range(4):  # tự giảm mini_batch_size nếu OOM (không đụng batch_size hiệu dụng)
        try:
            ...
            if isinstance(e, ImportError):
                ...
                finetune_info["reason"] = f"thiếu package lúc train: {e}"
                return model, finetune_info
            raise
    ...
    finetune_info.update({"used_finetune": True, "max_steps": max_steps,
...
def try_dev_eval(bm25, dense_model, dense_embeddings, all_chunks, train_data, train_positive,
    ...
    random.seed(SEED)  # re-seed CỐ Ý ở đây (xem giải thích ở set_all_seeds()/CONFIG): giữ 300
                        # câu dev-eval LUÔN CỐ ĐỊNH bất kể USE_WARMUP/USE_FINETUNE bật hay tắt.
...
# ==============================================================================
def main() -> None:
    # BẢN SỬA #7: seed toàn cục NGAY ĐẦU — trước MỌI lời gọi random (Bước 3 sinh nhãn dùng
    # random qua build_train_pairs gián tiếp không, nhưng Bước 4 chắc chắn có). Xem giải
    # thích đầy đủ ở khai báo SEED/set_all_seeds() trong CONFIG.
    set_all_seeds(SEED)
    checkpoint("Bắt đầu")
    print(f"  Cache/model tải về: {CACHE_DIR}")
    print(f"  SEED={SEED} · USE_WARMUP={USE_WARMUP} · USE_FINETUNE=True (luôn thử fine-tune "
          f"nếu đủ dữ liệu+thời gian trên bản .py)")
    ...
    # {qid: {"question": str, "answer": str}} — lọc chặt cả kiểu dữ liệu (đã gặp thật: một
    # số câu có "answer" là list, không phải string, ở bản dữ liệu Kaggle). KHÔNG gộp vào
    # mẫu dev-eval (Bước 6) — dev-eval chỉ dùng train.json gốc để giữ tín hiệu chọn cấu
    # hình đáng tin (xem phần 2.2 trong PHAN_TICH_KY_THUAT.md).
    ...
    dense_model, finetune_info = finetune_or_load_dense(train_positive, train_data_for_pairs,
    ...
    # LƯU Ý: dùng train_data GỐC (không gộp warmup) làm mẫu dev-eval — xem lý do ở comment Bước 3.
    top_n_answer, use_reranker, use_adaptive, eval_info = try_dev_eval(
        bm25, dense_model, dense_embeddings, all_chunks, train_data, train_positive,
    ...
    # BẢN SỬA #7 — "sổ thí nghiệm": 1 dòng JSON/lần chạy, APPEND (không ghi đè), đủ để so
    # sánh nhiều lần chạy sau này (khác seed, khác USE_WARMUP, khác phần cứng...) mà không
    # phải lục lại log console. Ghi ở cuối cùng nên không mất kể cả nếu Bước 4-7 chạy lâu.
    record = {
        "ts": time.strftime("%Y-%m-%d %H:%M:%S"), "seed": SEED, "use_warmup": USE_WARMUP,
        "n_warmup_used": n_warmup_used, "hardware": "local (.py)",
        ...
        "finetune_max_steps": finetune_info["max_steps"],
        "finetune_mini_batch_final": finetune_info["mini_batch_size_final"],
        "finetune_calib_time_s": finetune_info["calib_time_s"],
