...
def rerank(question: str, candidates: list, reranker_model, reranker_tokenizer,
    ...
    try:
        ...
        return reranked + candidates[max_candidates:]  # phần ngoài top max_candidates giữ nguyên thứ tự cũ
    except RuntimeError as e:
...
def answer_question(question: str, bm25, dense_model, dense_embeddings, all_chunks, top_n: int,
                     reranker_model=None, reranker_tokenizer=None) -> str:
...
def try_dev_eval(bm25, dense_model, dense_embeddings, all_chunks, train_data,
    ...
    best_n, best_m, best_use_rerank = 3, -1.0, False
    ...
    print(f"  => chọn TOP_N_ANSWER={best_n}, dùng reranker={best_use_rerank} (METEOR={best_m:.4f})")
    return best_n, best_use_rerank
...
def main() -> None:
    ...
    top_n_answer, use_reranker = try_dev_eval(bm25, dense_model, dense_embeddings, all_chunks,
